...
def extract_category(soup):
    ...
    try:
        # Strategy 1: Look for breadcrumb with id
        breadcrumb = soup.find('div', {'id': 'wayfinding-breadcrumbs_feature_div'})
        if breadcrumb:
            links = breadcrumb.find_all('a', {'class': 'a-link-normal'})
            if links:
                categories = [link.get_text(strip=True) for link in links]
                return ' › '.join(categories)
        
        # Strategy 2: Alternative breadcrumb selector
        breadcrumb = soup.find('ul', {'class': 'a-unordered-list a-horizontal a-size-small'})
        if breadcrumb:
        ...
        # Strategy 3: Look for any element with breadcrumb-like structure
        breadcrumb_items = soup.find_all('li', {'class': 'a-breadcrumb-divider'})
        if breadcrumb_items:
            parent = breadcrumb_items[0].find_parent('ul')
            if parent:
    ...
    try:
        ...
        for product_div in product_divs:
            try:
                ...
                # Extract "bought in past month" metric
                bought_elem = product_div.find('span', string=re.compile(r'bought in past month', re.IGNORECASE))
                if bought_elem:
                    product_data['bought_past_month'] = bought_elem.get_text(strip=True)
                else:
                    product_data['bought_past_month'] = 'N/A'
...
def extract_plans_for_all_products(products, session=None):
    ...
    for idx, product in enumerate(products, 1):
        ...
        # Extract protection plans and category
        plans, category = extract_protection_plans(asin, name, session=session)
        ...
        result = {
            ...
            'Bought in Past Month': product.get('bought_past_month'),
...
def create_hyperlinked_excel(data, filename):
    ...
    column_widths = {
        ...
        'F': 12,   # Discount %
        'G': 10,   # Rating
        'H': 18,   # Number of Reviews
        'I': 22,   # Bought in Past Month
        'J': 20,   # Product Link
        'K': 35,   # Plan 1 Name
        'L': 12,   # Plan 1 Price
        'M': 15,   # Plan 1 Brand
...
def main():
    ...
    products_with_ratings = sum(1 for r in results if r.get('Rating') and r['Rating'] != 'N/A')
    products_with_discount = sum(1 for r in results if r.get('Discount %') and r['Discount %'] != 'N/A')
    
    # Step 4: Email report (if configured via env)
    try:
        # Create detailed email body with summary
        email_body = f"""Amazon Godrej Products Scraping Report
{'='*50}
...
• Products with protection plans: {with_plans}
• Total protection plans found: {total_plans}
• Products with ratings: {products_with_ratings}
• Products with discounts: {products_with_discount}

TIMESTAMP: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

The detailed Excel report is attached with the following columns:
- ASIN
- Product Name
- Category
...
- Protection Plans (up to 3 per product with Name, Price, Brand)

Please review the attached file for complete details.
"""
        
        sent = send_report(
            filename,
            subject=f"Godrej Scraping Report - {len(products)} Products - {timestamp}",
            body=email_body,
        )
        if sent:
            print("✓ Report emailed with summary")
    except Exception as e:
